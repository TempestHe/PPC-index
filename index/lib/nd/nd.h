...
inline void createOrderedIndexEdges (nd_Graph& graph, vector<vp>& el, vector<int>& xel, nd_Graph& orderedGraph) {
	...
	for (size_t u = 0; u < graph.size(); u++) {
		...
		// reorder
		// for(int k=0;k<orderedGraph[u].size(); ++k){
		// 	for(int l=k+1;l<orderedGraph[u].size(); ++l){
		// 		if(orderedGraph[u][k] > orderedGraph[u][l]){
		// 			swap(orderedGraph[u][k], orderedGraph[u][l]);
		// 			swap(el[starting_offset+k], el[starting_offset+l]);
		// 		}
		// 	}
		// }
